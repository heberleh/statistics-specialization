...
<li> [x] 50\% of houses in Ames are smaller than 1,499.69 square feet. </li>
...
# type your code for the Exercise here, and Run Document
sample_means50 %>%
  summarise(mean_mean = mean(x_bar), mean_med = median(x_bar), 
...
<ol>
<li> [x] Each element represents a mean square footage from a simple random sample of 10 houses. </li>
...
sample_means50 %>% summarize(mean_xbar=mean(x_bar), median_xbar=median(x_bar), sigma=sd(x_bar), q1=quantile(x_bar, 0.25), q3=quantile(x_bar,0.75), ic0=quantile(x_bar,0.025),ic1=quantile(x_bar,0.975))
...
sample_means150 %>% summarize(mean_xbar=mean(x_bar), median_xbar=median(x_bar), sigma=sd(x_bar), q1=quantile(x_bar, 0.25), q3=quantile(x_bar,0.75), ic0=quantile(x_bar,0.025),ic1=quantile(x_bar,0.975))
...
# type your code for this Exercise here, and Run Document
sample_means15 <- sample_n(ames, size = 15, replace = TRUE )
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <- rep_sample_n(ames,size=15, replace=TRUE, reps=2000) %>%
                  summarize(x_bar=mean(price))

ggplot(sample_means15, aes(x=x_bar)) + geom_histogram(binwidth = 6000)

sample_means15 %>% summarize(mean_xbar=mean(x_bar), median_xbar=median(x_bar), sigma=sd(x_bar), q1=quantile(x_bar, 0.25), q3=quantile(x_bar,0.75), ic0=quantile(x_bar,0.025),ic1=quantile(x_bar,0.975))
```
...
# type your code for this Exercise here, and Run Document
sample_means150 <- rep_sample_n(ames, size=150, replace=TRUE, reps=2000) %>%
                  summarize(x_bar=mean(price))

ggplot(sample_means150, aes(x=x_bar)) + geom_histogram(binwidth = 6000)
...
<ol>
<li> [x] The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). </li>
...
sample_means50 <- rep_sample_n(ames, size=50, replace=TRUE, reps=2000) %>%
                  summarize(x_bar=mean(price))
sample_means50 %>% summarize(sigma50=sd(x_bar))


sample_means150 <- rep_sample_n(ames, size=150, replace=TRUE, reps=2000) %>%
                  summarize(x_bar=mean(price))
sample_means150 %>% summarize(sigma150=sd(x_bar))
