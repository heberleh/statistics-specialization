...
nc %>%
  ggplot(aes(x = factor(habit), y = weight)) +
  geom_boxplot() + coord_flip()

nc %>%
  group_by(habit) %>%
  summarize(IQR=IQR(weight))
...
<li> [x] We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers. </li>
</ol>
```{r habit-weight-ci}
...
inference(y = weight, data = nc, statistic = "mean", type = "ci", method = "theoretical", conf_level = 0.90)
```
...
tapply(nc$mage, nc$mature, summary)
  

inference(y = mage, x = habit, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
...
nc %>%
  ggplot(aes(x = factor(mature), y = weight)) +
  geom_boxplot() + coord_flip()
...
nc %>%
  ...
  ggplot(aes(x = factor(whitemom), y = visits)) +
  geom_boxplot() + coord_flip()
  
inference(y = visits, x = whitemom, data = nc, statistic = "mean", type = "ht", null = 0,
          alternative = "less", method = "theoretical")

inference(y = visits, x = whitemom, data = nc, statistic = "mean", type = "ci", method = "theoretical")


```

    Despite there is a difference between number of visitis grouping by whitemom (p-value<0.05), the difference in a 95% confidence interval is of about only 1 visit. The difference exists, but it is not big enough to continue the investigation on other levels.
