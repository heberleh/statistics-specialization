...
```{r set-seed}
set.seed(557896412)                 # make sure to change the seed
```
...
samp %>% summarize(mean=mean(area), median=median(area), q1=quantile(area,0.25), q3=quantile(area,0.75), IQR=IQR(area))
```

      The distribution is nearly normal but slightly right skewed - the mean is greater than the median. 50% of values are between 1254 and 1712.75, making an IQR of 458.75.
...
<li> [x] The sample distribution must be nearly normal. </li>
</ol>
...
# type your code for the Exercise here, and Knit
params > 1401.484 & params < 1633.849
```
...
#ci %>%
#  summarize(prop_yes=sum(capture_mu=="yes")/n())
