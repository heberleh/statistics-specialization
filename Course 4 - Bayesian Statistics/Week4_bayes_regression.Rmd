...
* [x] 10 of the respondents make strictly less than 300 dollars per week
...
$$`r round(summary(m_wage_educ)$coefficients[1,1],3)` + `r round(summary(m_wage_educ)$coefficients[2,1],3)` \times \text{EDUC}$$
...
* [x] The residuals are strongly left skewed, hence the normal distribution of errors condition is not met.
...
* [x] Case 616 has a probably of close to 1 that it an outlier under the normal error model for regressing `lwage` on `iq`
