...
delta <- 1-0.95
qbeta(c(delta/2, 1-delta/2), shape1 = 1+x, shape2 = 1+n-x)
```
...
* [x] The probability that the true proportion of females lies in this interval is 0.95.
...
alfa <- a+x
beta <- b+n-x
qbeta(c(delta/2, 1-delta/2), shape1 = alfa, shape2 = beta)
```
...
* [x] approximately 0.03
* approximately 0.15
* approximately 0.50 -> wrong
* approximately 0.97
</div>
...
```{r}
s <- sample(brfss$sex, size = length(brfss$sex)*0.0999)
n <- length(s)
x <- sum(s == "Female")
...
sum(brfss$sex == "Female") / length(brfss$sex)
```
...
cr_i <- qgamma(c(delta/2, 1-delta/2), shape = alfa, rate = beta)
mean(cr_i)
cr_i
```
...
# priori <- qbeta(c(0.2), shape1 = a, shape2 = b)
# posteriori <- qbeta(c(0.2), shape1 = alfa, shape2 = beta)
# 
# pbeta(q = 0.2, shape1 = alfa, shape2 = beta)
# 
# posteriori
...
v = (mu_belif*(sd_belief^2) + n*mu_data*(sd_data^2))  /  (sd_belief^2+ n*(sd_data^2))

t = sqrt(((sd_belief^2)*(sd_data^2))    /   ((sd_belief^2)+n*(sd_data^2)))
