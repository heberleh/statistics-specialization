...
ggplot(data = nc_fullterm, aes(x = gained)) +
  geom_histogram(binwidth = 5)
```
...
```{r}
inference(y = weight, data = nc_fullterm, statistic = "mean", type="ci", method="theoretical", conf_level = 0.95)
```
...
* [x] There is a 95% chance that babies on average weigh 7.4 to 7.5 pounds. 
</div>
...
* [x] there is a 95% chance that their birth weight will be 5.4 to 9.5 pounds.
...
IQR(smoker$weight)
IQR(nonsmoker$weight)

median(smoker$weight)
median(nonsmoker$weight)
...
* [Wrong] Range of birth weights of female babies are roughly the same as that of male babies.
* [x] Both distributions are approximately symmetric.
...
* Not worth a bare mention
* [Wrong] Positive ???? if 3 to 10 is Positive according to the course, why not?
* [x] Strong
* Very Strong
</div>
...
* [x] there is a 95% chance that babies born to nonsmoker mothers are on average 0.11 to 0.54 pounds heavier at birth than babies born to smoker mothers. 
</div>
