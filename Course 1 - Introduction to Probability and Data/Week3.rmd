...
kobe_streak %>%
  summarize(iqr=IQR(length), median=median(length), mean=mean(length), min=min(length), max=max(length))
```
...
<li> [x] The shortest streak is of length 1.</li>
</ol>
...
shot_outcomes <- c("H", "M")
p_hit <- 0.45
sim_basket <- sample(shot_outcomes, size=133, replace=TRUE, prob = c(p_hit,1-p_hit))
table(sim_basket)
...
<ol>
<li> [x] The distributions look very similar. Therefore, there doesn't appear to be evidence for Kobe Bryant's hot hand. </li>
