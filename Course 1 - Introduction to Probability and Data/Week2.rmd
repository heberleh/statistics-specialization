...
<li> [x] The distribution is right skewed. </li> 
<li> [x] No flight is delayed more than 2 hours. </li> 
<li> [x] The distribution has several extreme values on the right side. </li>
...
sfo_feb_flights %>% summarise(mean=mean(arr_delay), median=median(arr_delay), IQR=IQR(arr_delay), n=n())

sfo_feb_flights %>% filter(arr_delay<0) %>% count()
sfo_feb_flights %>% count()
sfo_feb_flights %>% filter(arr_delay<0) %>% count() / sfo_feb_flights %>% count()
...
<ol>
<li> American Airlines - it is not AA.....  </li> 
...
<li> [x] Median would be more reliable as the distribution of delays is skewed. </li> 
...
<li> [x] There is an overall postive association between distance and average speed.</li> 
...
ggplot(nycflights, aes( y = avg_speed)) +
  geom_boxplot()
```
...
nycflights %>%
  summarize(
    percentage_delayed = sum(arr_type=="delayed")/n(),
    percentage_ontime = sum(arr_type=="on time")/n(), 
    delayed_ontime = sum(dep_type=="delayed" & arr_type=="on time")/sum(dep_type=="delayed")
    )
