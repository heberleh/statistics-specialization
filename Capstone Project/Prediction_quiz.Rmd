...
ames_test$price_pred <- exp(predict(model.AIC, newdata = ames_test))
mean(ames_test$price_pred)
```
...
<li> [x] When predicting to `ames_train`, the RMSE for `model.AIC` is higher than the RMSE for `model.full`.  However, when predicting to `ames_test`, the RMSE for `model.full` is higher.
...
resid.AIC <- ames_test$price - exp(predict(model.AIC, ames_test))
rsme.AIC <- sqrt(mean(resid.AIC^2))


# prediction with Train

resid.full.train <- ames_train$price - exp(predict(model.full, ames_train))
rsme.full.train <- sqrt(mean(resid.full.train^2))

resid.AIC.train <- ames_train$price - exp(predict(model.AIC, ames_train))
rsme.AIC.train <- sqrt(mean(resid.AIC.train^2))

data.frame(Data = c("test", "train"), 
           AIC = c(rsme.AIC, rsme.AIC.train), 
           Full = c(rmse.full, rsme.full.train))
...
model.BIC <- stepAIC(model.full, k = log(nrow(ames_train)), trace = FALSE)
#summary(model.BIC)
